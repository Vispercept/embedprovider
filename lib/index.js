...
const getSourceUri = (html) =>
  /src="(?<uri>.[^"]*)"/.test(html)
    ? /src="(?<uri>.[^"]*)"/.exec(html).groups.uri
    : '';
const getHrefUri = (html) =>
  /href="(?<uri>.[^"]*)"/.test(html)
    ? /href="(?<uri>.[^"]*)"/.exec(html).groups.uri
    : '';
const prependProtocol = (uri) => uri.replace(/^\/\//, 'https://');
const getHostname = (uri) => new URL(uri).hostname;
const getProvider = (hostname) =>
  /(?<provider>[\w-]+)\.\w+$/.exec(hostname).groups.provider;
const getDomain = (hostname) =>
  /(?<domain>[\w-]+\.\w+)$/.exec(hostname).groups.domain;
const findMappedProvider = (providerInfo, map) =>
  map.find(({ from: { domain } }) => providerInfo.domain === domain);
...
function embedprovider(embedCode, { customProviderMap } = {}) {
  if (!embedCode) return '';
  const map = customProviderMap
    ? providerMap.concat(...customProviderMap)
    : providerMap;
